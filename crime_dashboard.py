...
@st.cache_data
def load_data():
    CRB_clean = pd.read_excel('C:/Users/User/Desktop/Data analytics/Complete- crime rate- Bradford-1.xlsx', sheet_name=0)
    crime_map = {
    ...
    CRB_clean['Crime type label'] = CRB_clean['Crime type'].map(crime_map)
    CRB_clean['Last outcome label'] =CRB_clean['Last outcome category'].map(outcome_map)
    CRB_clean['Date'] = pd.to_datetime(CRB_clean['Year'].astype(str) + '-' + CRB_clean['Month'], format='%Y-%b')
    return CRB_clean
